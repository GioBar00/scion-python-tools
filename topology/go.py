...
from topology.common import (
    ...
    translate_features,
    SD_API_PORT,
    SD_CONFIG_NAME, join_host_port, CS_CLIENT_PORT, CS_SRV_PORT, CS_SVC_REDIRECT_PORT,
)
...
class GoGenerator(object):
    def __init__(self, args):
        ...
        """
        self.args = args
        self.log_dir = '/share/logs' if args.docker or args.kubernetes else 'logs'
        self.db_dir = '/share/cache' if args.docker or args.kubernetes else 'gen-cache'
        self.certs_dir = '/share/crypto' if args.docker or args.kubernetes else 'gen-certs'
    ...
    def _build_br_conf(self, topo_id, ia, base, name, v):
        config_dir = '/share/conf' if self.args.docker or self.args.kubernetes else base
        raw_entry = {
            'general': {
                'id': name,
    ...
    def _build_control_service_conf(self, topo_id, ia, base, name, infra_elem, ca):
        ...
        if self.args.kubernetes:
            raw_entry['quic'] = {
                'address_server': prom_addr(infra_elem['addr'], CS_SRV_PORT), # join_host_port('0.0.0.0', base_port),
                'address_client': prom_addr(infra_elem['addr'], CS_CLIENT_PORT),# join_host_port('0.0.0.0', base_port)
                'address_svcredirect': prom_addr(infra_elem['addr'], CS_SVC_REDIRECT_PORT), # join_host_port('0.0.0.0', base_port)
            }
        if ca:
    ...
    def generate_disp(self):
        if self.args.docker or self.args.kubernetes:
    ...
    def _gen_disp_docker(self):
        for topo_id, topo in self.args.topo_dicts.items():
            ...
            if self.args.kubernetes:
                elem_ids =list(topo.get("control_service", {}))
            for k in elem_ids:
                disp_id = 'disp_%s' % k
    ...
    def _build_disp_conf(self, name, topo_id=None):
        ...
        srv_addresses = self._build_srv_addresses(self.args.docker or self.args.kubernetes, name, topo_id)
        tomlDict = {
            'dispatcher': {
                'id': name,
            },
    ...
    def _metrics_entry(self, infra_elem, base_port):
        ...
        # if self.args.kubernetes:
        #     a = join_host_port('0.0.0.0', base_port)
        return {
            'prometheus': a,
        }
