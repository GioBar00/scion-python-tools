...
from topology.net import (
    PortGenerator,
    SubnetGenerator, KubernetesService
)
...
class TopoGenerator(object):
    ...
    def _reg_addr(self, topo_id: TopoID, elem_id, addr_type):
        if self.args.kubernetes:
            return KubernetesService(f"{elem_id.replace('_', '-')}-svc.scion")
    ...
    def _gen_srv_entry(self, topo_id, as_conf, conf_key, def_num, nick,
        ...
        for i in range(1, count + 1):
            ...
            if not self.args.docker and not self.args.kubernetes:
                port = self.args.port_gen.register(elem_id)

            # if self.args.kubernetes:
            #     d = {
            #         'addr': f"scion-{elem_id}-svc.scion:{port}",
            #     }
            # else:
            d = {
