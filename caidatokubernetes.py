...
def main():
    ...
    with open("topology-generator-scion/default.topo") as f:
    ...
    for link in links:
        if link['from'] in core_ids and link['to'] in core_ids:
            portA = '-' + string.ascii_uppercase[math.floor(as_intf_state[link['from']] / MAX_PORTS_PER_BR)] + '#' + str(as_intf_state[link['from']])
