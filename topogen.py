...
import os.path
import shutil
# TODO: fails because the service addresses in the handcrafted config have - and in our generated files we have _.
# if we generate the br and cs config as well, it'd be okay.
# SCION
...
from topology.config import (
    ConfigGenerator,
    ConfigGenArgs,
    DEFAULT_TOPOLOGY_FILE, DEFAULT_KUBECFG_FILE, DEFAULT_KUBE_NAMESPACE,
)
...
def add_arguments(parser):
    ...
    parser.add_argument('-k', '--kubernetes', action='store_true',
                        help='Create a kubernetes configuration')
    parser.add_argument('--kube-push-config', action='store_true',
                        help='Pushes the created configurations to kubernetes config maps. This requires a valid '
                             'kubeconfig.yaml to be present or set via --kube-config.')
    parser.add_argument('--kube-config', default=DEFAULT_KUBECFG_FILE,
                        help='The kubernetes configuration file (default: kubeconfig.yaml) that the config is deployed '
                             'to.')
    parser.add_argument('--kube-ns', default=DEFAULT_KUBE_NAMESPACE,
                        help='The kubernetes namespace (default: scion). The namespace will be created if it does not'
                             'exist yet.')
